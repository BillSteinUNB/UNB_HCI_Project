...
interface HomeProps { onNavigate: (screen: ScreenName) => void; }
...
export const HomeScreen: React.FC<HomeProps> = ({ onNavigate }) => {
  ...
  const NavButton = ({ title, desc, icon: Icon, target, colorClass }: any) => (
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 p-8">
      ...
      <div className="grid grid-cols-2 grid-rows-2 gap-6 flex-1 mb-8">
        <NavButton title="Find a Room" desc="Classrooms, labs, and faculty offices" icon={Search} target={ScreenName.FIND_ROOM} colorClass="bg-white border-gray-200 hover:border-[#C41230] text-[#C41230]" />
        <NavButton title="Today's Events" desc="Lectures, career fairs, and workshops" icon={Calendar} target={ScreenName.EVENTS} colorClass="bg-white border-gray-200 hover:border-[#C41230] text-[#1A1A1A]" />
